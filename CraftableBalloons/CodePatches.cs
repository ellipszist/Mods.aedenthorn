...
namespace CraftableBalloons
{
	public partial class ModEntry
    {
        ...
        public static class Game1_pressUseToolButton_Patch
        {
            public static bool Prefix()
            {
                ...
                if(npc != null)
                {
                    ...
                    if (oldHeldObj is ColoredObject c && c.ItemId == balloonKey)
                    {
                        Game1.player.reduceActiveItemByOne();
                        if (npc.modData.TryGetValue(colorKey, out var color))
                        {
                            var oldObj = new ColoredObject(balloonKey, 1, GetColor(color));
                            ...
                            Game1.player.ActiveObject = new ColoredObject(balloonKey, 1, GetColor(color));
                        }
                        npc.modData[colorKey] = MakeColorString(c.color.Value);

                    }
                    else if (npc.modData.TryGetValue(colorKey, out var color))
                    {
                        var oldObj = new ColoredObject(balloonKey, 1, GetColor(color));
        ...
        public static class Object_drawInMenu_Patch
        {
            ...
            {
                if (!Config.ModEnabled || __instance.ItemId != balloonKey || __instance.color.Value != new Color(6,6,6))
                    return;
                __state = __instance.color.Value;
                __instance.color.Value = Utility.GetPrismaticColor();
            }
            public static void Postfix(ColoredObject __instance, SpriteBatch spriteBatch, Vector2 location, float scaleSize, float transparency, float layerDepth, Color? __state)
            {
                ...
                spriteBatch.Draw(itemData.GetTexture(), location, new Rectangle(16, 0, 16, 16), Color.White * transparency, 0f, Vector2.Zero, 4f * scaleSize, SpriteEffects.None, layerDepth + 1);
                if(__state != null)
        ...
        public static class Farmer_draw_Patch
        {
            ...
            {
                if (!Config.ModEnabled || !TryGetBalloonColor(__instance, out Color color))
                    return;
                DrawBalloon(b, color, __instance.Position + new Vector2(0, +__instance.yJumpOffset * 2), __instance.FacingDirection, characterMovement.TryGetValue(__instance, out var data) ? data.vel : Point.Zero, __instance.StandingPixel.Y);
            }
        }
        ...
        public static class Pet_draw_Patch
        {
            public static void Postfix(NPC __instance, SpriteBatch b)
            {
                if (!Config.ModEnabled || !TryGetBalloonColor(__instance, out Color color))
                    return;
                DrawBalloon(b, color, __instance.Position + new Vector2(32, +__instance.yJumpOffset * 2 + 32), __instance.FacingDirection, characterMovement.TryGetValue(__instance, out var data) ? data.vel : Point.Zero, __instance.StandingPixel.Y);
            }
        }
        ...
        public static class InventoryPage_receiveLeftClick_Patch
        {
            ...
            {
                if (!Config.ModEnabled || !__instance.portrait.containsPoint(x, y) || (Game1.player.CursorSlotItem is not null && Game1.player.CursorSlotItem.ItemId != balloonKey))
                    return true;
                var oldCursorItem = Game1.player.CursorSlotItem;
                if (oldCursorItem is ColoredObject c && c.ItemId == balloonKey)
                {
                    ...
                    if (Game1.player.modData.TryGetValue(colorKey, out var color))
                    {
                        var oldObj = new ColoredObject(balloonKey, 1, GetColor(color));
                        ...
                        Game1.player.CursorSlotItem = new ColoredObject(balloonKey, 1, GetColor(color));
                    }
                    Game1.player.modData[colorKey] = MakeColorString(c.color.Value);

                }
                else if (Game1.player.modData.TryGetValue(colorKey, out var color))
                {
                    Game1.player.CursorSlotItem = new ColoredObject(balloonKey, 1, GetColor(color));
                    Game1.player.modData.Remove(colorKey);
                }
                else
                {
                    return true;
        ...
        public static class FarmerRenderer_draw_Patch
        {
            public static void Prefix(FarmerRenderer __instance, SpriteBatch b, Vector2 position, Vector2 origin, float layerDepth, int facingDirection, Color overrideColor, float rotation, float scale, Farmer who)
            {
                ...
                if (!Config.ModEnabled || !FarmerRenderer.isDrawingForUI || !who.modData.TryGetValue(colorKey, out var color))
                    return;
                ...
                b.Draw(tex, position + origin + __instance.positionOffset + new Vector2(24 + yoff, -16), new Rectangle(0, 16, 16, 16), GetDisplayColor(color), rotation, origin, scaledPixelZoom, SpriteEffects.None, FarmerRenderer.GetLayerDepth(layerDepth, FarmerRenderer.FarmerSpriteLayers.Base, false));
