    ...
    public class PuzzlePiece
    {
        ...
        public void Initialize()
        {
            ...
            for (int x = 0; x < tiles.GetLength(0); x++)
            {
                ...
                {
                    if (tiles[x, y] != null)
                        _tiles[x, y] = StardewDropsGameMenu.instance.tileDict[tiles[x, y].Value.X, tiles[x, y].Value.Y];
                }
            }
            for (int i = 0; i < 4; i++)
            {
                ...
                if(p != null)
                {
                    ...
                    {
                        if (p[j] != null)
                            t[j] = StardewDropsGameMenu.instance.tileDict[p[j].Value.X, p[j].Value.Y];
                    }
                    tf.SetValue(this, t);
                }
            }
            ...
            {
                if (connected[i] != null)
                    _connected[i] = StardewDropsGameMenu.instance.pieces[connected[i].Value.X, connected[i].Value.Y];
            }
        }
        ...
        public void Draw(SpriteBatch b, Vector2 offset, float zoom, Dictionary<TileSheet, Texture2D> dict, StardewDropsGameMenu menu)
        {
