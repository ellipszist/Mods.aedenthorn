...
namespace PlantAll
{
    public partial class ModEntry
    {
        ...
        {
            return item != null && item.Stack > 0 && (item.Category == -74 || item.Category == -19) && !(item as Object).isSapling() && !Object.isWildTreeSeed(item.ItemId);
        }
        ...
        private static void GetPlaceable(Item item, GameLocation location, int ox, int oy, int x, int y, List<Point> placeables)
        {
            ...
            for (int i = tiles.Count - 1; i >= 0; i--)
            {
                ...
                if (item.canBePlacedHere(location, new Vector2(tiles[i].X, tiles[i].Y))
