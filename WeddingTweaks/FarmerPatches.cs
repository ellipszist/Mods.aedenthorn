    ...
    public static class FarmerPatches
    {
        ...
        public static bool Farmer_getChildren_Prefix(Farmer __instance, ref List<Child> __result)
        {
            try
            {
                ...
                if (ModEntry.startingLoadActors && Environment.StackTrace.Contains("LoadActors") && !Environment.StackTrace.Contains("addActor") && !Environment.StackTrace.Contains("Dialogue") && !Environment.StackTrace.Contains("checkForSpecialCharacters") && Game1Patches.lastGotCharacter != null && __instance != null)
                {
